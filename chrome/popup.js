...
function initPopup() {
  getSettings().then(function() {
    if (document.readyState === "compvare" || "interactive") {
...
/* getSettings - (1) Gets local storage settings, (2) sets local settings var to local storage settings, (3) resolves promise when compvare  */
  ...
  chrome.tabs.query(queryInfo, function(tabs) {
    ...
    if (url) {
      ...
      getSettings().then(function() {
        ...
        if (index > -1) {
          document.querySelector("input[name=addtowhitelist]").checked = true
        }
      })
    }
  })
...
/* updateWhiteList - (1) Update URL white list settings with user input (2) save settings (3) send updated settings to tab.js to modify active tab blur css */
